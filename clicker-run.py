...
def get_user_input():
    ...
    while True:
        ...
        except ValueError:print(Fore.RED + "Invalid input. Please try again.");continue
...
def perform_clicks(positions, repeat_count, time_out, repeat_frequency):
    ...
    for i in range(repeat_count):
        ...
        for pos in positions:
            ...
            if 'left' in pos[2]:
                if exit_: print(Fore.RED+ "\nexiting...");time.sleep(2);exit()
...
def on_press(key):
    ...
    if key == Key.esc: print(Fore.YELLOW + "Stopping script."); exit_ = True; return False  # Stop listener

if __name__ == "__main__":
    repeat_count, time_out, repeat_frequency = get_user_input()
    positions = load_mouse_positions()
    with KeyboardListener(on_press=on_press) as listener: perform_clicks(positions, repeat_count, time_out, repeat_frequency); listener.join()
